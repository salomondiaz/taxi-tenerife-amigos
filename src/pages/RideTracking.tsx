...
const RideTracking = () => {
  ...
  useEffect(() => {
    if (!currentRide) {
      ...
      if (currentRide.status === "pending") {
        // Mostrar verificación de teléfono si no está verificado
        if (!isPhoneVerified) {
  ...
  }, [currentRide, navigate, setCurrentRide, isPhoneVerified]);
  ...
  const handleVerificationComplete = () => {
    ...
    // Continuar con la búsqueda del conductor
    if (currentRide?.status === "pending") {
      toast({
        title: "Buscando conductor",
        description: "Estamos localizando al conductor más cercano",
      });
    }
  };
